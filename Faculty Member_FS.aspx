<%@ Page Title="Faculty Search" Language="C#" MasterPageFile="~/master.Master" AutoEventWireup="true" CodeBehind="Faculty Member_FS.aspx.cs" Inherits="Eastern_Uni.Faculty_Member_FS" %>
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">

    
    <link rel="shortcut icon" href="image/Other/favicon.ico" />
    <link href="CSS/Common.css" rel="stylesheet" />
    <link href="CSS/demo.css" rel="stylesheet" />
    <link href="CSS/bjqs.css" rel="stylesheet" />
    <link href="CSS/Home.css" rel="stylesheet" />


    <style type="text/css">
        .auto-style3 {
            width: 79px;
            height: 36px;
        }
        .auto-style4 {
            width: 249px;
            height: 36px;
        }
        .auto-style5 {
            height: 37px;
        }
        .auto-style6 {
            width: 249px;
            height: 37px;
        }
        .auto-style7 {
            width: 8px;
            height: 36px;
        }
        .auto-style8 {
            height: 37px;
            width: 8px;
        }
        .auto-style9 {
            height: 37px;
            width: 79px;
        }
    </style>


</asp:Content>



<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">

    <div id="ContentBody">

       
        






        <!-- start of second phase -->
        <div id="second">
            

            <div id="s_container" style="background-image: url('image/Other/Content_Banner.png'); background-repeat: no-repeat">
                 <br /><br /><br />
                  <table style="width: 91%; height: 56px;">
                    <tr>
                        <td class="auto-style19">

                        </td>
                        <td class="auto-style15" style="text-align: right;font-weight: bold; font-size: 25px">
                                  Faculty Search
                
                        </td>
                    </tr>
                </table>

               
                <br /><br />
              


                <form id="Form1" runat="server">


                     <div >
                        <br />
                        <br />
                        <table style="width:98%">
                        <tr>
                            <td class="auto-style10">
                                &nbsp;</td>
                            
                            <td>
  <asp:Label ID="errorMsg" runat="server" Text="" ForeColor="Red" Font-Bold="True"></asp:Label> 
                   

                    <table style="width:100%">

                                    <tr>
                                        <td style="text-align: right;width:40%">
                                            <asp:Label ID="lblFaculty" runat="server" Font-Size="20px" Text="Faculty : "  ></asp:Label>
                                        </td>
                                        <td>
                                    <asp:DropDownList runat="server" AutoPostBack="True" 
                             Height="20px" Width="260px" 
                             style="position:relative; z-index:auto; top: 0px; left: 0px;"  
                             ID="ddlFaculty"   >
                        </asp:DropDownList>
  <asp:DropDownList runat="server" AutoPostBack="True" 
                             Height="20px" Width="260px" 
                             style="position:relative; z-index:auto; top: 0px; left: 0px;"  
                             ID="ddlDesignation" Visible="false"   >
                        </asp:DropDownList>
                                            <asp:Label ID="Label5" runat="server" Text="*" Font-Bold="True" Font-Size="Large" ForeColor="#FF3300"></asp:Label> 
                                        </td>
                                    </tr>

                                    <tr>
                                        <td style="text-align: right;width:40%">

                                            <asp:Label ID="lblDesignation" runat="server" Font-Size="20px" Text="Designation : " ></asp:Label>
                                        </td>
                                        <td>
                                    <asp:DropDownList runat="server" AutoPostBack="True" 
                            Height="20px" Width="260px" 
                            style="position:relative; z-index:auto; top: 0px; left: 0px;" 
                            ID="ddlDeg"   >
                        </asp:DropDownList>

                                            <asp:Label ID="Label8" runat="server" Font-Bold="True" Font-Size="Large" ForeColor="#FF3300" Text="*"></asp:Label>
                                        </td>
                                    </tr>

                                    <tr>
                                        <td style="text-align: right;width:40%">
                                            <asp:Label ID="lblName" runat="server" Font-Size="20px" Text="Name : "></asp:Label>
                                        </td>
                                        <td>


                                    <asp:TextBox ID="txtName" runat="server" Width="260px"></asp:TextBox> 

                                        </td>
                                    </tr>

                                    <tr  >
                                        <td colspan="2" style="text-align: center">
                                     <asp:Button ID="Button1" runat="server" Text="Search" OnClick="Button1_Click" />
                                            </td>
                                    </tr>

                                </table>
                            </td>
                            
                        </tr>

                    </table>
                          &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
              
                    </div>

                    

                    <br />
                    <asp:Label ID="Label1" runat="server" Text="" ></asp:Label> 
                   




                 <asp:GridView ID="gvDepartment" runat="server" AutoGenerateColumns="False" BackColor="White"
                                                AllowPaging="true" PageSize="20" BorderColor="#CCCCCC" BorderStyle="None" BorderWidth="1px"
                                                CellPadding="3" DataKeyNames="Serial_no" EmptyDataText="No Data Found" ShowHeaderWhenEmpty="True"
                                                Width="95%" style="margin-left: 15px"  OnRowEditing="gvDepartment_RowEditing" OnPageIndexChanging="gvDepartment_PageIndexChanging">
                                                <Columns>


                                                        <asp:TemplateField HeaderText="Faculty Member(s)"  ItemStyle-Width="65%">
                                                        <ItemTemplate>
                                                            <asp:Label ID="Label2" runat="server" Text="Name :" style="font-size:medium;font-weight: bold;"></asp:Label>  <asp:Label ID="lbName" runat="server" Text='<%#Bind("Name") %>' style="font-size:medium;font-weight: bold;" ></asp:Label>
                                                            <br />
                                                            <asp:Label ID="Label3" runat="server" Text="Designation :" style="font-weight: bold;"></asp:Label> 
                                                             <asp:Label ID="lblSpDeg" runat="server" Text='<%#Bind("SpecialDesignationPost") %>'></asp:Label>
                                                            <asp:Label ID="lblDesignation" runat="server" Text='<%#Bind("Designation") %>'></asp:Label>
                                                            <br /><asp:Label ID="lblAdditionalResponsibility" runat="server" Text='<%#Bind("AdditionalResponsibility") %>'></asp:Label>
                                                            <br />
                                                             <asp:Label ID="Label4" runat="server" Text="Qualification :" style="font-weight: bold;"></asp:Label> <asp:Label ID="lblBackground" runat="server" Text='<%#Bind("AcademicBackground") %>'></asp:Label>
                                                            <br />
                                                            <asp:Label ID="Label5" runat="server" Text="Research Interest:" style="font-weight: bold;"></asp:Label>
                                                            <br /><asp:Label ID="lblResearch" runat="server" Text='<%#Bind("ResearchInterest") %>'></asp:Label>

                                                            <br />
                                                            <asp:Label ID="Label6" runat="server" Text="Phone :" style="font-weight: bold;"></asp:Label><asp:Label ID="lblPhone" runat="server" Text='<%#Bind("Phone") %>'></asp:Label>
                                                            <br />
                                                           <asp:Label ID="Label7" runat="server" Text="Email :" style="font-weight: bold;"></asp:Label> <asp:Label ID="lblEmail" runat="server" Text='<%#Bind("Email") %>'></asp:Label>

                                                            <br />
                                                            <asp:LinkButton ID="lnkDetail" runat="server" CausesValidation="false" CommandName="Edit"
                                                                Text="Click for Detail"></asp:LinkButton>
                                                            
                                                        </ItemTemplate>
                                                        <ItemStyle HorizontalAlign="left" />
                                                        <HeaderStyle HorizontalAlign="left" />
                                                        </asp:TemplateField>

                                                       

                                                <asp:TemplateField>
                                                <ItemTemplate>
                                                    <asp:Image ID="Image2" runat="server" Width="225px" Height="150px" ImageUrl='<%# Bind("PictureLocation") %>'  />
                                                
                                                </ItemTemplate>
                                                <ItemStyle HorizontalAlign="Center" />
                                                <HeaderStyle HorizontalAlign="Center" />
                                                </asp:TemplateField>

                                                    

                                                    

                                                </Columns>
                                                <EmptyDataRowStyle Font-Bold="True" HorizontalAlign="Center" />
                                                <FooterStyle BackColor="White" ForeColor="#000066" />
                                                <HeaderStyle BackColor="#A3A5BA" Font-Bold="True" ForeColor="#000000" Font-Size="Larger" />
                                                <PagerStyle BackColor="White" ForeColor="#000066" HorizontalAlign="Left" />
                                                <RowStyle ForeColor="#000066" />
                                                <AlternatingRowStyle BackColor="AliceBlue" />
                                                <SelectedRowStyle BackColor="#669999" Font-Bold="True" ForeColor="White" />
                                            </asp:GridView>


                    <br />



                    <asp:GridView ID="gvFacultyDtl" runat="server" AutoGenerateColumns="False" BackColor="White"
                                                AllowPaging="true" PageSize="20" BorderColor="#CCCCCC" BorderStyle="None" BorderWidth="1px"
                                                CellPadding="3" DataKeyNames="Serial_no" EmptyDataText="No Data Found" ShowHeaderWhenEmpty="True"
                                                Width="100%">
                                                <Columns>


                                        <asp:TemplateField  HeaderText="Detailed Information" >
                                                        <ItemTemplate>
                                                            <asp:Image ID="Image2" runat="server"  ImageUrl='<%# Bind("PictureLocation") %>' Width="225px" Height="150px" /> <br />
                                                            <br />
                                                            <asp:Label ID="Label2" runat="server" Text="Name :" style="font-size:medium;font-weight: bold;" ></asp:Label>
                                                               
                                                           
                                                             <asp:Label ID="lbName" runat="server" Text='<%#Bind("Name") %>' style="font-size:medium;font-weight: bold;" ></asp:Label>
                                                            <br />

                                                             <asp:Label ID="Label14" runat="server" Text="Telephone(office) :" style="font-size:medium;font-weight: bold;" ></asp:Label>
                                                           
                                                            <asp:Label ID="lblPhone" runat="server" Text='<%#Bind("Phone") %>'></asp:Label>
                                                            <br />


                                                            <asp:Label ID="Label12" runat="server" Text="Email :" style="font-size:medium;font-weight: bold;" ></asp:Label>
                                                           
                                                            <asp:Label ID="lblEmail" runat="server" Text='<%#Bind("Email") %>'></asp:Label>
                                                            <br />
                                                           
                                                            <asp:Label ID="Label13" runat="server" Text="Academic Background :" style="font-size:medium;font-weight: bold;" ></asp:Label>
                                                            <br />
                                                            <asp:Label ID="lblLastDegree" runat="server" Text='<%#Bind("AcademicBackground") %>'></asp:Label>
                                                            <br /><br />
                                                            <asp:Label ID="Label11" runat="server" Text="Research Interest :" style="font-size:medium;font-weight: bold;" ></asp:Label>
                                                            <br />
                                                            
                                                            <asp:Label ID="lblYear" runat="server" Text='<%#Bind("ResearchInterest") %>'></asp:Label>
                                                            <br />
                                                            
                                                            
                                                            
                                                            <br />

                                                            

                                                            <br />


                                                            <asp:Label ID="Label9" runat="server" Text="Teaching Experience :" style="font-size:medium;font-weight: bold;" ></asp:Label>
                                                            <br />
                                                            <asp:Label ID="lblTeachingExp" runat="server" Text='<%#Bind("TeachingExp") %>'></asp:Label>
                                                            <br /><br />


                                                            <asp:Label ID="Label10" runat="server" Text="Administrative Position Held (if any):" style="font-size:medium;font-weight: bold;" ></asp:Label>
                                                            <br />
                                                           <asp:Label ID="lblAdminsPos" runat="server" Text='<%#Bind("AdminsPos") %>'></asp:Label>
                                                            <br />
                                                            <br />
                                                            <asp:Label ID="Label8" runat="server" Text="Selected Publications :" style="font-size:medium;font-weight: bold;" ></asp:Label>
                                                            <br />


                                                            <asp:Label ID="lblUniversity" runat="server" Text='<%#Bind("Publications") %>'></asp:Label>
                                                        </ItemTemplate>
                                                        <ItemStyle HorizontalAlign="left" />
                                                        <HeaderStyle HorizontalAlign="left" />
                                                        </asp:TemplateField>

                                               
                                                    
                                                         

                                                

                                                    

                                                    

                                                </Columns>
                                                <EmptyDataRowStyle Font-Bold="True" HorizontalAlign="Center" />
                                                <FooterStyle BackColor="White" ForeColor="#000066" />
                                                <HeaderStyle BackColor="#A3A5BA" Font-Bold="True" ForeColor="#000000" Font-Size="Larger"  />
                                                <PagerStyle BackColor="White" ForeColor="#000066" HorizontalAlign="Left" />
                                                <RowStyle ForeColor="#000066" />
                                                <AlternatingRowStyle BackColor="AliceBlue" />
                                                <SelectedRowStyle BackColor="#669999" Font-Bold="True" ForeColor="White" />
                                            </asp:GridView>

             <!--      <asp:GridView ID="gvFacultyDtl1" runat="server" AutoGenerateColumns="False" BackColor="White"
                                                AllowPaging="true" PageSize="20" BorderColor="#CCCCCC" BorderStyle="None" BorderWidth="1px"
                                                CellPadding="3" DataKeyNames="FacultyID" EmptyDataText="No Data Found" ShowHeaderWhenEmpty="True"
                                                Width="100%">
                                                <Columns>


                                                        <asp:TemplateField  ItemStyle-Width="75%">

                                                          
                                                        <ItemTemplate>
                                                            <asp:Image ID="Image2" runat="server" Height="160px" ImageUrl='<%# Bind("PictureLocation") %>' Width="140px" />
                                                            <br /><br />
                                                            
                                                           
                                                        </ItemTemplate>

                                                       

                                                        <ItemStyle HorizontalAlign="left" />
                                                        <HeaderStyle HorizontalAlign="left" />
                                                        </asp:TemplateField>

                                                       

                                               

                                                    

                                                    

                                                </Columns>
                                                <EmptyDataRowStyle Font-Bold="True" HorizontalAlign="Center" />
                                                <FooterStyle BackColor="White" ForeColor="#000066" />
                                                <HeaderStyle BackColor="#A3A5BA" Font-Bold="True" ForeColor="#000000" />
                                                <PagerStyle BackColor="White" ForeColor="#000066" HorizontalAlign="Left" />
                                                <RowStyle ForeColor="#000066" />
                                                <AlternatingRowStyle BackColor="AliceBlue" />
                                                <SelectedRowStyle BackColor="#669999" Font-Bold="True" ForeColor="White" />
                                            </asp:GridView>

                    -->
                    <div runat="server" id="result" >

                        <div style="background-color: #99CCFF">
                            <table style="border: thin solid #c34500" border="1">

                    <asp:DataList ID="datalist_e" runat="server">

                        <ItemTemplate>

                            <tr >
                                <td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                     &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                     &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;  
         
                                </td>

                                
                            </tr>




                            <tr >
                                
                                <td><strong style="font-size:larger;text-transform:uppercase;"><%#Eval("Name") %></strong> </td>
                                <td rowspan="7" style="border: 4px solid #6699FF">
                                     <asp:Image ID="Image2" runat="server" Height="160px" ImageUrl='<%# Bind("PictureLocation") %>' Width="140px" />
                                </td>
                            </tr>



                            <tr >
                                  <td><%#Eval("Designation") %></td>
                                
                            </tr>

                              <tr>
                                  <td><%#Eval("AcademicBackground") %></td>
                            </tr>

                            <tr>
                                <td>Research Area:<%#Eval("ResearchInterest") %></td>
                            </tr

                            <tr>
                                <td>Phone:&nbsp;<%#Eval("Phone") %></td>
                            </tr




                            <tr>
                                <td>Email:&nbsp;<%#Eval("Email") %></td>
                            </tr>



                            <tr>
                               <!-- <td><a href="FacultyReport.aspx" style="text-decoration: none; color: #C34500;">Detail</a></td>-->
                                <td><a href="<%#Eval("Detailslink") %>" style="text-decoration: none; color: #C34500;">Detail</a></td>
                            </tr>
                            <tr style="visibility: hidden">
                                <td> 
                                    <asp:Label ID="lblFacultyID" runat="server" Text='<%#Bind("FacultyID") %>'></asp:Label>
                                </td>
                            </tr>
                            <tr >
                                <td colspan="3">
                                    
                                    <hr />
                                </td>

                            </tr>


                        </ItemTemplate>

                    </asp:DataList>



                </table>

                        </div>

                         

                <br />
                <br />


           


                <table  style="position:relative ;left:250px;">
                <tr>
                    <td>
                        <asp:Button ID="btnfirst" runat="server" Font-Bold="true" Text="<<" Height="31px"
                            Width="73px" OnClick="btnfirst_Click" /></td>
                    <td>
                        <asp:Button ID="btnprevious" runat="server" Font-Bold="true" Text="<" Height="31px"
                            Width="73px" OnClick="btnprevious_Click" /></td>
                    <td>
                        <asp:Button ID="btnnext" runat="server" Font-Bold="true" Text=">" Height="31px"
                            Width="73px" OnClick="btnnext_Click" /></td>
                    <td>
                        <asp:Button ID="btnlast" runat="server" Font-Bold="true" Text=">>" Height="31px"
                            Width="73px" OnClick="btnlast_Click" /></td>
                </tr>
            </table>




                    </div>

                </form>

            </div>


            
        </div>


    </div>


</asp:Content>


